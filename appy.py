...
image = cv2.imread("imgs_rostros/oscar_toaquiza_0504121344.png")
...
while True:
    ...
    if face_locations != []:
           for face_location in face_locations:
            face_frane_encodings = face_recognition.face_encodings(frame, known_face_locations=[face_loc])[0]
            result = face_recognition.compare_faces([face_image_encodings], face_frane_encodings)
            print("Result", result)
            if result[0] == True:
                text = "Oscar"
                color = (125, 220, 0)
